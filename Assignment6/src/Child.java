/**
 * This program demonstrates the Child subclass, which extends the abstract Ticket superclass. 
 * This class represents a Child ticket class and contains a constructor, a method for 
 * calculating the ticket price for a child, and a toString method to display the ticket
 * information.
 * @author hudson
 */
public class Child extends Ticket {
	...
	public Child(String movieN, String rating, int date, int time, String f) {
		super(movieN,rating,date,time,f);
	}
	
	/**
	 * This method overrides the calculateTicketPrice method of the abstract Ticket superclass.
	 * If the movie rating is not PG13, R, or NR, it sets the price to 10.75 if the movie was seen 
	 * during the evening and 5.75 otherwise. If the feature is an IMAX then the movie price is 
	 * increased by 2, and if it is a 3D movie, then it is increased by 1.50, otherwise it remains the same. 
	 * Finally, the tax rate is applied to the ticket price and added to the ticket price and the ticket 
	 * price is returned.
	...
	public double calculateTicketPrice() {
		if(super.getRating() == "PG13" || super.getRating() == "R" || super.getRating() == "NR") {
			return -1;
	 ...
	 * because children do not have Id's.
	...
	public String toString() {
		return	super.getType() + " " + super.getFeature() + " Movie: " + super.getMovieName() + " Rating: " +
